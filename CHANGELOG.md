...
Release candidate; Fix: renaming a device's site in Forward now moves the existing device instead of creating a duplicate under the new site - the case-insensitive device-identity fix in `v2.9.8` did not cover a site change alone. A new Scope Reconciliation action, **Merge site-relabel duplicates**, repairs pairs already created by this bug (keeps the older device, moved to the new site). Fix: scope reconciliation and orphan/uncovered pruning are now site-aware instead of name-only, so a stale same-name copy left at the old site is never confused with the live device at the new one, and a pair awaiting merge is never pruned out from under the operator. Fix: the dependency preview and drift report show a model's pending-removal count once instead of once per query map that touches it. Feature: a Health warning fires when a bundled query's live signature no longer matches what NetBox last confirmed (the `v2.9.7` outage class), and the release preflight now refuses to ship an `@query` parameter change without a changelog mention. Feature: the support bundle carries a `diagnostics` section (uncovered-device causes, identity bindings, rename/case-collision counts, NQE map bindings, ingestion-issue detail) and `duplicate_device_names`, and `config_backup_delivery` status; `ConfigBackupError` now names the pipeline stage (`resolve`, `fetch_remote`, `branch`, `nqe_fetch`, `push`) a failure happened in, so an instant config-backup failure says where. No migration; upgrade, then run Scope Reconciliation → **Merge site-relabel duplicates**, then re-sync. No `@query` signature changes in this release - no republish needed.
...
Fix: **a device the database refuses to delete no longer wedges the whole run's convergence bookkeeping**. A device is deleted through the branch collector rather than the shared delete path, so a `ProtectedError` there was never converted into the skip every other model gets and was recorded as a FAILED row - and one failed row blocks baseline promotion permanently, which puts every drift figure back to "Not measured" for the whole deployment. Declining to delete a device is minor; wedging convergence over it is not. It is now a skip that names the NetBox row and the models still referencing it. Fix: **Prune orphans no longer removes shared catalogues and global IPAM**. Rows dropped by device-tag scope became deletes whenever pruning was enabled, with no model policy consulted, so a deployment saw repeated protected-delete skips against the software-version catalogue. Pruning removes devices and their sites - that is the feature - but "this device left tag scope" is not a statement about a catalogue every device shares. All three delete producers now read one allowlist, and a test requires every model with a delete handler to be classified by it. Fix: **the drift report says how much of the estate it measured**. "Not measured" covered three unrelated situations - a report older than the measurement feature, a preview that compared nothing, and ordinary partial coverage - and only the first is fixed by re-running Preview Dependencies. The report now shows how many models were compared out of how many, names the ones it could not compare, and distinguishes a stale report from a defect. "Drifted models" is also counted over the models actually compared, so a partial measurement is never read as a whole-estate one. Compatibility: **tested and validated on NetBox 4.6.8**, exercised through the full suite, the installed-artifact check and the upgrade path rather than asserted. The supported range is unchanged at 4.6.x from 4.6.5. Also: the merge's tree-model detection no longer depends on a single upstream base class, the release command reports the release it actually performed instead of failing after a successful publish, and the scale gate declines to answer on a loaded machine rather than reporting a regression that is not there. No migration; upgrade and re-run the sync.

## v2.8.2

Fix: **a sync no longer tries to delete the shared catalogues, global IPAM and operator-gated rows that Forward stopped reporting**. Baseline reconciliation has refused to remove sites, device types, platforms, manufacturers, device roles, prefixes, VLANs, VRFs, devices and the software-version catalogue since 2.7.13, for reasons written out beside the list: shared catalogues may be Device Type Library imports, global IPAM is never pruned by device scope, and device and site removal is operator-gated through Scope Reconciliation -> Prune orphans because absence from a query result is not evidence an object is gone. The Forward-diff delete path enforced none of it and deleted every one of them unattended. One deployment reached the delete for a VRF, a device type and three sites in a single sync, and only a database PROTECT constraint stopped it - a site or device type with no children has nothing holding it. Both paths now read one allowlist, and a test requires every model with a delete handler to be classified by it, so a model added later cannot inherit "deletable" by omission. Renames are deliberately unaffected: a row whose identity key changed is superseded by a row written in the same batch, and that delete still applies. Operator-visible change: a device or site Forward stops reporting now stays in NetBox until someone prunes it. Fix: **a protected-delete skip names the NetBox row it is about**. Five skips reading `dcim.site row processing skipped (...; still referenced by dcim.device)` were each correct and none actionable - two byte-identical to each other - because everything that would identify the row is a name or a slug and is redacted before it persists. The record now carries the NetBox primary key, which resolves to the object page. Also: the release checks name the release and commit they measured instead of silently reporting on the wrong one, and the hash-pinned release toolchain moves to `cryptography` 50.0.0. No migration; upgrade and re-run the sync.
