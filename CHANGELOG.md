...
Release candidate; Diagnosability and convergence release. Fix: **a merge failure that names no field now names its rule**. A non-field validation error reports `__all__` — a field name meaning "no field" — so a customer's `ipam.ipaddress` merge failure was indistinguishable from any other cause. NetBox interpolates the message before raising, putting the address inside the string, so it cannot be persisted as-is: known rules now resolve to slugs this plugin defines, and anything uncatalogued is reduced to wording with every value-bearing token masked, so a rule nobody has seen yet is still legible on sight. Fix: **the ingestion log download contains the issues behind the run** — message, exception and structured diagnosis, with an explicit truncation flag past 200 rows. It previously described everything around a failure except the failure, and the exception text was discarded before anything persisted it, so searching server logs for it could not have matched on any deployment. Fix: **a row NetBox refuses on its own validation rule no longer wedges the baseline**. Any failed row blocks baseline promotion, and re-running cannot change a validation rejection because the rule belongs to the destination rather than the incoming data, so a few permanently unsatisfiable addresses could stop thousands of correct changes from ever being attested. Those rows are recorded and skipped; integrity and other errors still fail and still block, because those are usually ordering or contention where a retry is right. Fix: **the one ingestion that cannot be deleted no longer reads as an error** — it is the baseline, the durable record of what has already converged, and NetBox protects it deliberately; the refusal now says so and is raised as a warning, after being reported as a defect twice. No migration and no operator step; upgrade and re-run the sync.
