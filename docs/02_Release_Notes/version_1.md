...
- `SD-518` - Add support for Forward Networks tables in NetBox device UI.
...
- `SD-475` - Support duplicate prefixes, if no prefix is provided by Forward Networks use NetBox's Global VRF as default.
...
- `SA-494` - URL for the link to Forward Networks using `sn` instead of `snHw`.
