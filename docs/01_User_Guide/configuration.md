...
Optional routing sync is enabled by default through `PLUGINS_CONFIG["forward_netbox"]["enable_bgp_sync"] = True`. When the supported optional NetBox plugins are installed, the sync form exposes the `netbox_routing.bgppeer`, `netbox_routing.bgpaddressfamily`, `netbox_routing.bgppeeraddressfamily`, `netbox_routing.ospfinstance`, `netbox_routing.ospfarea`, `netbox_routing.ospfinterface`, `netbox_routing.prefixlistentry`, `netbox_routing.communitylistentry`, `netbox_routing.routemapentry`, and `netbox_peering_manager.peeringsession` maps. The three policy maps (prefix lists, community lists, route maps) are opt-in and config-derived: they parse `device.files.config`, store each policy under its configured name when every device defines it the same way and as `<name>@<device>` for each divergent definition (because `netbox-routing` names these objects globally), and require *Publish Bundled Queries* once after upgrading on org-backed deployments. The `netbox-routing` maps are the primary native BGP/OSPF targets; the `netbox-peering-manager` map creates an overlay session linked to the BGP peer. Set the flag to `False` only when routing synchronization is not required. The routing queries use explicit local identity when Forward provides it, then apply conservative native-query inference from reciprocal Forward peer evidence. During planning, routing diagnostics report BGP neighbors skipped because no explicit or safely inferred local AS exists, unsupported BGP address families, and OSPF rows skipped because no unique process-level local router ID can be inferred safely.
