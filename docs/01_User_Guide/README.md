...
```bash
pip install /path/to/forward_netbox-0.8.6.2-py3-none-any.whl
```

Alternatively, install directly from the GitHub source archive:

```bash
pip install /path/to/forward_netbox-0.8.6.2.tar.gz
```

If you mirror the package into a private Python index, pin the same release version:

```bash
pip install --pre forward-netbox==0.8.6.2
```
...
| --- | --- | --- |
| `v0.8.6.2` | `4.5.9` validated; `4.5.x` only | Current release; hardens issue and job-log rendering so unexpected nested payload objects stay JSON-safe in the UI and API |
