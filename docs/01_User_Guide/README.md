...
```bash
pip install /path/to/forward_netbox-0.8.1.1-py3-none-any.whl
```

Alternatively, install directly from the GitHub source archive:

```bash
pip install /path/to/forward_netbox-0.8.1.1.tar.gz
```

If you mirror the package into a private Python index, pin the same release version:

```bash
pip install --pre forward-netbox==0.8.1.1
```
...
| --- | --- | --- |
| `v0.8.1.1` | `4.5.9` validated; `4.5.x` only | Current release; fixes virtual chassis NQE output so NetBox receives a member position with virtual chassis assignments |
