...
```bash
pip install /path/to/forward_netbox-2.4.0-py3-none-any.whl
pip install /path/to/forward_netbox-2.4.0.tar.gz
```

If you mirror the package into a private Python index, pin the same release version:

```bash
pip install forward-netbox==2.4.0
```
...
| --- | --- | --- |
| `v2.4.0` | `4.6.4` required (4.5.x dropped); needs netbox-branching `1.1.0+` | Current release; Fix: the "Import SNMP Endpoints as Devices" toggle now renders on the source form (the field shipped in 2.3.2 but was not in any fieldset, so it never showed), letting operators enable endpoint import from the GUI. |
