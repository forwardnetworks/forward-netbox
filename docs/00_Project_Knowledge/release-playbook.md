...
Before any `1.1.x` release, refresh field-scale evidence against REDACTED's
dataset and enforce the gate:

```bash
export FORWARD_SMOKE_DATASET_LABEL=redacted
...
`passed`, not labeled as REDACTED's dataset, missing required matrix steps, or
