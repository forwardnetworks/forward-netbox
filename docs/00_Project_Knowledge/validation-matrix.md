...
| --- | --- |
| Documentation only | `invoke harness-check`, `invoke docs` |
| Query map or NQE helper change | `invoke harness-check`, `invoke lint`, `invoke test`, update built-in NQE reference |
| Forward API client change | `invoke lint`, `invoke check`, `invoke test` |
| Sync planning or branch budget change | `invoke lint`, `invoke check`, `invoke test`, local Docker sync smoke test |
| NetBox model adapter change | `invoke lint`, `invoke check`, `invoke test`, targeted local Docker sync |
