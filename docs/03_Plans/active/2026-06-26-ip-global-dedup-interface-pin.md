...
## Goal
Partner's run resolved only 6 of ~520 Mgmt_-tag primary IPs (514 unresolved) and
...
- No release until Partner re-runs and confirms (per instruction).
...
LIVE PROBE (Partner, before/after): per-device Django-shell classifier of the 514
...
- LIVE FINDING (customer network, current snapshot): old vs fixed IPv4 query both
  ...
  sample. So this defect is real but is NOT the dominant cause of Partner's 514
