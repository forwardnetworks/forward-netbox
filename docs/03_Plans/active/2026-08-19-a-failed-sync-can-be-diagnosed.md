...
## A note on this change's own history

The first version of this work used a device hostname copied from the
deployment's own screenshot as a negative test fixture - in the test that exists
to prove such values are never emitted. It reached `main` before the pre-tag
audit caught it.

`main` was reset to the last clean commit and the content re-landed through this
pull request, which is the recovery that preserves pull-request provenance;
rewriting the offending commit in place would have orphaned it by SHA and
refused every future release. The fixtures are now RFC-reserved: `.invalid`
hostnames (RFC 2606) and TEST-NET-1 addresses (RFC 5737), which cannot collide
with any real fabric.
