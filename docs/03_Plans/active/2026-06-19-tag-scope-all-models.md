...
- Prefix breadth: prefixes derived from in-scope devices' routing/forwarding
  tables are network-wide (a tagged router knows remote routes). On CustomerOrg, routing-
...
- `forward_netbox/queries/forward_prefixes_ipv4.nqe` and
  ...
  scope. CustomerOrg: v4 routing 91,859 -> connected 26,223 scoped; v6 connected 49.
...
- Live CustomerOrg: `forward_vrfs` unscoped=425 vs scoped(Prod_Core)=288 — the tag filter
...
CustomerOrg scale: ~91,859 -> ~26,223 under the Prod_Core tag, i.e. ~65k prefix deletes on
