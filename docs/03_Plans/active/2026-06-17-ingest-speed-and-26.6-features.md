...
- Keep customer data, credentials, network IDs, and live execution keys out of
  docs and tests. Live validation is local (the CustomerOrg network, via the
  untracked `.env`).
...
3. **NQE diffs for updates.** ✅ Wired, verify coverage.
   ...
   baseline.
   - This is the primary update-side load reduction. Confirm on live CustomerOrg that
...
- **Live CustomerOrg runtime evidence** (local, via the untracked `.env`):
