...
1. Prove compatibility-window safety for compatibility branch-state cache while preserving upgrade
   behavior.
2. Gather ADP `0.9.1` real-run evidence before making deeper architectural moves.
...
2. `0.9.1` ADP dataset stability validation (real-run evidence baseline)
...
## Workstream 2: ADP `0.9.1` Real-Run Validation

### Objective

Collect real stability/performance evidence on the ADP dataset before deeper
...
For this deferred-risk tranche, ADP real-run evidence is required in addition to
local CI.
...
- Chosen: require ADP real-run validation before claiming stability improvements
