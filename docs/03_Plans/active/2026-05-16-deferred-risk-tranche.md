...
1. Prove compatibility-window safety for compatibility branch-state cache while preserving upgrade
   behavior.
2. Gather CustomerOrg `0.9.1` real-run evidence before making deeper architectural moves.
...
2. `0.9.1` CustomerOrg dataset stability validation (real-run evidence baseline)
...
## Workstream 2: CustomerOrg `0.9.1` Real-Run Validation

### Objective

Collect real stability/performance evidence on the CustomerOrg dataset before deeper
...
For this deferred-risk tranche, CustomerOrg real-run evidence is required in addition to
local CI.
...
- Chosen: require CustomerOrg real-run validation before claiming stability improvements
