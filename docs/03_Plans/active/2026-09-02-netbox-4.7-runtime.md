...
- **Live Forward validation**, which no unit test covers: the plugin's own
  ...
  networks, resolved the configured network, fetched 100 snapshots, and ran an async
  NQE query against its newest processed snapshot returning **5228 device rows** - the
