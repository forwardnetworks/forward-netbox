...
## Field-reported 2.6.3 regression: unpinned query IDs disable a full sync

A customer upgraded 2.6.1 to 2.6.3, ran **Resolve to Query ID** as directed, and
got a sync that reported Completed / Validation Passed with zero changes, no
branch, and a Drift report stuck at "not measured". The support bundle and job
log establish the whole chain:

1. **Resolve to Query ID** stores a `query_id` but leaves the commit unresolved.
   All 34 maps report `commit_binding: latest_commit`, `has_commit_id: false`.
2. `query_execution_contract.py` marks a contract ineligible when `commit_id` is
   empty or `"head"` (`unresolved_full_commit`). A map that deliberately tracks
   the latest commit is therefore never eligible for a full run, even though
   resolving current head at each sync is the documented behaviour of a
   path-bound map.
3. `query_fetch_execution.py` raises `ForwardQueryError` for any map that is not
   `full_eligible`, so the model is skipped. `query_contract_preflight` reported
   `consistent: false` with 34 issues; only 2 NQE query calls ran for 29 enabled
   models, against 3,176 correctly-scoped devices.
4. `single_branch_executor.py` treated the resulting empty workload as "no
   changes", recording a completed, branchless, zero-change ingestion. A run
   where every fetch failed was indistinguishable from a converged one.
5. Drift stayed "not measured" because no ingestion ever applied anything.

### Fixed here

**An empty workload set caused by fetch failures now fails closed.**
`failed_model_strings` decides whether an empty workload means convergence or
wholesale failure, and the executor raises with the failing model list instead
of recording a clean no-op. Extracted as a helper so the decision is unit
tested rather than buried in `run()`.

### Still to fix in 2.6.4

- **Resolve head to a concrete commit** so a latest-commit binding is
  full-eligible. `get_committed_nqe_query` already resolves head commit and
  source from the repository index, and the maps store both path and query ID,
  so this needs no schema change and no operator step: on 2.6.4 the customer
  re-runs the sync, with no rebinding and no re-running Resolve to Query ID.
  Keep the parameter-compatibility guard: the contract must still refuse a
  revision whose declared parameters do not match.
- **Stop the support bundle blanket-sanitizing every warning.**
  `_sanitize_log_rows` replaces the message of every warning-or-worse row with
  `SAFE_FAILURE_LOG_MESSAGE`, so the bundle showed 34 identical "The operation
  failed" lines while the UI showed the real reasons. The stripped messages were
  plugin-authored and already safe. Preserve them, and keep redacting only
  exception-derived text.
- **Do not warn about diff-contract incompatibility while diffs are dormant.**
  Most maps' only issue was `diff:missing_diff_commit`, which is noise in a
  full-only release and buried the two real `full:unresolved_full_commit` cases.
