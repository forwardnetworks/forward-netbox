...
CORRECTION, 2026-08-01: the paragraph above is wrong about why the anchors sat
where they did, and the claim was repeated into PR #112 and the 2.6.11 release
notes. The anchors were not stale through neglect. They were pinned deliberately
so that `_require_security_bootstrap` would see `3b6fe4d`, the only commit that
has ever touched the bootstrap files, in its diff. Moving them fixed the
retention failure and broke that check instead, which is how `v2.6.11` burned.
Four releases were indeed hand-tagged and did skip the bridge step, but that is
not why provenance failed. See
`docs/03_Plans/active/2026-08-01-release-2.6.12-provenance-bootstrap.md`.
