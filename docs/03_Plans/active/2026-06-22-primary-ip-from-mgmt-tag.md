...
names the management interface. Partner's spec: a device tag `Mgmt_<iface>` (e.g.
`Mgmt_Vl211`) → primary IP = the address on that interface (`Vlan211`). Partner
...
## Defaults chosen (flag Partner if different)
...
- **Opt-in, default off** — lands green without changing any existing sync's
  behavior; Partner enables it per sync once validated.
