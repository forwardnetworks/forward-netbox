...
netbox-dlm 0.9.1 was released upstream on 2026-08-14. We pinned 0.8.0.
Every runtime gate in this plugin compares the installed optional-plugin
version against an explicit validated set and fails closed, so until 0.9.1 is
in those sets an upgrade is
...
- Our `max_version` and netbox-dlm's are now identical at 4.6.99. Worth raising
  upstream before either side moves to 4.7, because whoever goes first blocks
  the other.
