...
Three things, all Python/packaging (no query change, so no validation-org republish):
...
- **Hardware-notice alias skips deferred to a fast-follow (2.5.4):** that fix is
  a new NQE query variant needing an validation-org republish; keeping it out of 2.5.3
