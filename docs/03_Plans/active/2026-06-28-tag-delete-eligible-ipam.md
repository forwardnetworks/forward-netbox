...
hand. This is the safe first step toward IPAM scope cleanup (Blake's request):
...
- Tag-first over auto-delete: Blake asked to "start by tagging"; deletion of
...
- Tag slug `forward-delete-eligible` (over Blake's loose `delete_eligible`) to
