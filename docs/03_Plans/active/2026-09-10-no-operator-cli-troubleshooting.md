...
- Phase 2 (done in 2.9.6, see `2026-09-12-fast-baseline-aci-routing-audits-tranche.md`): the nine audits are pages under the sync's **Audits** entry; the six that ask Forward run as button jobs, the three that read only NetBox compute on open. `forward_interface_vlan_audit` carries the interface and device primary keys in its rows for the links and persists nothing.
