...
- Evidence base commit: `4a587e2214143215cda298bdbc45b70542f313a3`

- [x] `final-tree-full-gate` - `rtk env FORWARD_NETBOX_DOCKER_PROJECT=forward-netbox-release-gate FORWARD_NETBOX_POSTGRES_DATA_PATH=netbox-postgres-data FORWARD_NETBOX_WORKER_AUTORELOAD=0 NETBOX_VER=v4.6.8 FORWARD_NETBOX_UPGRADE_FROM_VERSION=2.9.1 FORWARD_NETBOX_PATTERN_PARITY_UNVERIFIED=1 invoke ci` passed on the final 2.9.2 tree with exit status 0: 382 tests OK, 70 tests OK, 2587 tests OK, 1 tests OK across 3040 tests in total, and the upgrade leg seeded under the previous release and upgraded onto the tested runtime with its rows surviving.
...
- [x] `exact-runtime-artifact` - `rtk env FORWARD_NETBOX_DOCKER_PROJECT=forward-netbox-release-gate FORWARD_NETBOX_POSTGRES_DATA_PATH=netbox-postgres-data FORWARD_NETBOX_WORKER_AUTORELOAD=0 NETBOX_VER=v4.6.8 FORWARD_NETBOX_UPGRADE_FROM_VERSION=2.9.1 FORWARD_NETBOX_PATTERN_PARITY_UNVERIFIED=1 FORWARD_NETBOX_HOST_PORT=54387 NETBOX_URL=http://127.0.0.1:54387 invoke artifact-test` passed with exit status 0 against the installed `forward_netbox-2.9.2-py3-none-any.whl` on NetBox 4.6.8, Branching 1.1.3, Python 3.14, with every installed route returning 200, all plugin migrations applying cleanly with no drift, and a validated SBOM.
