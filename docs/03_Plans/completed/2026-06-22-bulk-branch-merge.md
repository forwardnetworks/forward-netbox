...
Make large syncs (Partner/CustomerOrg: ~1.04M changes/sync) survive on NetBox 4.6.3 +
...
grounded in the real 1.1.0 source and Partner's actual plan_preview volumes.
...
- Partner's sync: 1,043,183 changes / 163 shards; dcim.interface 539k, macaddress
...
execution ledger records totals — acceptable for Partner's single-writer
...
branch once via `merge_branch`/`bulk_merge_changes`. Collapses Partner's 163
...
- `apply_engine_bulk.emit_branch_object_changes(created, updated)` — after each
  ...
  **`.using(branch.connection_name)`** (core.ObjectChange is not branchable, so
  ...
  required by the plugin merge (it reads only ObjectChange) and Partner auto-merges
