...
Reported by the design partner: in-scope Avocent endpoints were not syncing.
...
- No customer data committed; validated against the approved validation org only.
...
was validated live against the approved validation org: 5123 rows = all devices + 121
Avocent endpoints (manufacturer "Avocent", model "Avocent ACS 8000"/"Cyclades
ACS 6000", role "Console Server", tag-scoped).

## Validation
Live NQE run against the validation org; `test_endpoints_import`; `test_query_registry`,
