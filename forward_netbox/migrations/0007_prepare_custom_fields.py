...
def prepare_custom_fields(apps: "apps", schema_editor: "BaseDatabaseSchemaEditor"):
    """Forward migration to prepare forward_netbox custom fields and links."""
    ...
    create_custom_field(
        ...
        [Device, Site],
        cf_type="type_object",
        object_type=apps.get_model("forward_netbox", "ForwardSource"),
    )
    create_custom_field(
        apps,
        "forward_branch",
        ...
        [Device, Site],
        cf_type="type_object",
        object_type=apps.get_model("forward_netbox", "ForwardBranch"),
    )
    cl, _ = apps.get_model("extras", "CustomLink").objects.update_or_create(
        defaults={
            "link_text": "{% if object.custom_field_data.forward_source is defined %}{% set SOURCE_ID = object.custom_field_data.forward_source %}{% if SOURCE_ID %}Forward{% endif %}{% endif %}",
            "link_url": '{% if object.custom_field_data.forward_source is defined %}{% set SOURCE_ID = object.custom_field_data.forward_source %}{% if SOURCE_ID %}{% set BASE_URL = object.custom_fields.filter(related_object_type__model="forwardsource").first().related_object_type.model_class().objects.get(pk=SOURCE_ID).url %}{{ BASE_URL }}/inventory/devices?options={"filters":{"sn": ["like","{{ object.serial }}"]}}{% endif %}{%endif%}',
            "new_window": True,
...
def cleanup_custom_fields(apps: "apps", schema_editor: "BaseDatabaseSchemaEditor"):
    """Reverse migration to prepare forward_netbox custom fields and links."""
    for custom_field_name in ["forward_source", "forward_branch"]:
...
class Migration(migrations.Migration):
    dependencies = [
        ...
        (
            "forward_netbox",
            "0006_alter_forwardtransformmap_target_model",
        ),
    ]
    operations = [
