...
def rename_forward_branch_cf(
    apps: "apps_type", schema_editor: "BaseDatabaseSchemaEditor"
):
    """Forward migration to rename forward_branch CustomField to forward_ingestion."""
    modify_custom_field(
        apps,
        "forward_branch",
        "forward_ingestion",
        "Forward Last Ingestion",
        apps.get_model("forward_netbox", "ForwardIngestion"),
    )


def rename_forward_ingestion_cf(
    apps: "apps_type", schema_editor: "BaseDatabaseSchemaEditor"
):
    """Forward migration to rename forward_ingestion CustomField to forward_branch."""
    modify_custom_field(
        apps,
        "forward_ingestion",
        "forward_branch",
        "Forward Last Sync",
        apps.get_model("forward_netbox", "ForwardBranch"),
    )
...
def clean_redisual_branch_data(
    ...
    try:
        branch_content_type = ContentType.objects.get(
            app_label="forward_netbox", model="forwardbranch"
        )
...
def change_transform_maps(
    ...
    * Slugify platform when using it for DeviceType slug.
    """
    ContentType = apps.get_model("contenttypes", "ContentType")
    TransformMap = apps.get_model("forward_netbox", "ForwardTransformMap")
    TransformField = apps.get_model("forward_netbox", "ForwardTransformField")
    RelationshipField = apps.get_model("forward_netbox", "ForwardRelationshipField")
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(migrations.RunPython.noop, rename_forward_ingestion_cf),
        ...
        migrations.AlterField(
            model_name="forwardrelationshipfield",
            name="template",
            field=models.TextField(blank=True, default=""),
        ),
        migrations.AlterField(
            model_name="forwardtransformfield",
            name="template",
            field=models.TextField(blank=True, default=""),
        ),
        migrations.RunPython(rename_forward_branch_cf, migrations.RunPython.noop),
