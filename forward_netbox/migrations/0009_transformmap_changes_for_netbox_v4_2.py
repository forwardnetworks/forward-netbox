...
from forward_netbox.utilities.transform_map import get_transform_map


if TYPE_CHECKING:
    from django.apps import apps
    from django.db.backends.base.schema import BaseDatabaseSchemaEditor
    from forward_netbox.models import ForwardTransformField, ForwardTransformMap
...
def get_current_transform_map(
    ...
    return apps.get_model("forward_netbox", "ForwardTransformMap").objects.get(
...
def get_current_map_field(apps: "apps", target_model: str) -> "ForwardTransformField":
    ...
    return apps.get_model("forward_netbox", "ForwardTransformField").objects.get(
        source_field="mac",
...
def create_transform_map(apps: "apps", schema_editor: "BaseDatabaseSchemaEditor"):
    ...
    with contextlib.suppress(
        apps.get_model("forward_netbox", "ForwardTransformMap").DoesNotExist
    ):
    ...
    ForwardTransformField = apps.get_model("forward_netbox", "ForwardTransformField")
    field_data = {
        "source_field": "duplex",
        "target_field": "duplex",
    ...
    try:
        ForwardTransformField.objects.get(**field_data)
    except ForwardTransformField.DoesNotExist:
        ForwardTransformField(**field_data).save()
    # endregion
...
def delete_transform_map(apps: "apps", schema_editor: "BaseDatabaseSchemaEditor"):
    ...
    # region - MAC Address
    apps.get_model("forward_netbox", "ForwardTransformField")(
        coalesce=False,
        source_field="mac",
        target_field="mac_address",
    ...
    apps.get_model("forward_netbox", "ForwardTransformField").objects.get(
        source_field="duplex",
        target_field="duplex",
        transform_map=get_current_transform_map(apps, "interface"),
    ).delete()
...
class Migration(migrations.Migration):
    dependencies = [
        ...
        ("forward_netbox", "0008_prepare_transform_maps"),
    ]
