...
from forward_netbox.forms import ForwardIngestionFilterForm
from forward_netbox.forms import ForwardIngestionMergeForm
from forward_netbox.forms import ForwardRelationshipFieldForm
from forward_netbox.forms import ForwardSnapshotFilterForm
from forward_netbox.forms import ForwardSourceFilterForm
from forward_netbox.forms import ForwardSourceForm
from forward_netbox.forms import ForwardSyncForm
from forward_netbox.forms import ForwardTransformFieldForm
from forward_netbox.forms import ForwardTransformMapCloneForm
from forward_netbox.forms import ForwardTransformMapForm
from forward_netbox.forms import ForwardTransformMapGroupForm
...
class ForwardSourceFormTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        # Create a test ForwardSource instance for form tests
        cls.forward_source = ForwardSource.objects.create(
            name="Test Forward Source",
            type=ForwardSourceTypeChoices.LOCAL,
            url="https://test.forward.local",
            status=DataSourceStatusChoices.NEW,
            parameters={"auth": "test_token", "verify": True, "timeout": 30},
        )

    def test_fields_are_required(self):
        form = ForwardSourceForm(data={})
    ...
    def test_valid_local_source_form(self):
        form = ForwardSourceForm(
            data={
                ...
                "verify": False,
                "timeout": 45,
                "description": "Test local Forward source",
    ...
    def test_valid_remote_source_form(self):
        form = ForwardSourceForm(
            data={
                ...
                "type": ForwardSourceTypeChoices.REMOTE,
                "url": "https://remote.forward.local",
                "timeout": 60,
                "description": "Test remote Forward source",
                "comments": "Test comments",
            }
        )
        self.assertTrue(form.is_valid(), form.errors)
    ...
    def test_form_initializes_existing_parameters(self):
        ...
        form = ForwardSourceForm(instance=self.forward_source)
    ...
    def test_remote_source_creates_last_snapshot(self):
        ...
        self.assertEqual(ForwardSnapshot.objects.count(), 0)
        ...
        last_snapshot = ForwardSnapshot.objects.filter(
    ...
    def test_fieldsets_dynamic_behavior_consistency(self):
        ...
        form_local_1 = ForwardSourceForm(
            data={"type": ForwardSourceTypeChoices.LOCAL}
        )
        form_local_2 = ForwardSourceForm(
            data={"type": ForwardSourceTypeChoices.LOCAL}
        )
        ...
        form_remote_1 = ForwardSourceForm(
            data={"type": ForwardSourceTypeChoices.REMOTE}
        )
        form_remote_2 = ForwardSourceForm(
            data={"type": ForwardSourceTypeChoices.REMOTE}
        )
    ...
    def test_fieldsets_source_type_changes_parameters_fieldset(self):
        ...
        form_local = ForwardSourceForm(data={"type": ForwardSourceTypeChoices.LOCAL})
        form_remote = ForwardSourceForm(
            data={"type": ForwardSourceTypeChoices.REMOTE}
        )
...
class ForwardRelationshipFieldFormTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        ...
        cls.transform_map_group = ForwardTransformMapGroup.objects.create(
        ...
        cls.transform_map = ForwardTransformMap.objects.create(
    ...
    def test_fields_are_required(self):
        form = ForwardRelationshipFieldForm(data={})
        self.assertFalse(form.is_valid(), form.errors)
    ...
    def test_form_initialization_with_existing_instance_no_data(self):
        ...
        # Create an existing ForwardRelationshipField instance
        relationship_field = ForwardRelationshipField.objects.create(
        ...
        form = ForwardRelationshipFieldForm(instance=relationship_field)
...
class ForwardTransformFieldFormTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.source = ForwardSource.objects.create(
            name="Test Source",
    ...
    def test_fields_are_required(self):
        form = ForwardTransformFieldForm(data={})
        self.assertFalse(form.is_valid(), form.errors)
    ...
    def test_form_initialization_with_existing_instance_no_data(self):
        ...
        # Create an existing ForwardTransformField instance
        transform_field = ForwardTransformField.objects.create(
            transform_map=self.transform_map,
            source_field="hostname",
            target_field="name",
            coalesce=True,
        ...
        # Initialize form with existing instance but no data
        form = ForwardTransformFieldForm(instance=transform_field)
...
class ForwardTransformMapGroupFormTestCase(TestCase):
    def test_fields_are_required(self):
        form = ForwardTransformMapGroupForm(data={})
        self.assertFalse(form.is_valid(), form.errors)
        self.assertIn("name", form.errors)

    def test_fields_are_optional(self):
        form = ForwardTransformMapGroupForm(data={"name": "Test Group"})
        self.assertTrue(form.is_valid(), form.errors)
...
class ForwardTransformMapFormTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.transform_map_group = ForwardTransformMapGroup.objects.create(
            name="Test Group", description="Test group description"
    ...
    def test_fields_are_required(self):
        form = ForwardTransformMapForm(data={})
        self.assertFalse(form.is_valid(), form.errors)
        self.assertIn("name", form.errors)
        self.assertIn("source_model", form.errors)
    ...
    def test_group_is_optional(self):
        ...
        ForwardTransformMap.objects.get(
...
class ForwardTransformMapCloneFormTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.transform_map_group = ForwardTransformMapGroup.objects.create(
            name="Test Group", description="Test group description"
    ...
    def fields_are_required(self):
        form = ForwardTransformMapCloneForm(data={})
        self.assertFalse(form.is_valid(), form.errors)
        self.assertIn("name", form.errors)
...
class ForwardSnapshotFilterFormTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.source = ForwardSource.objects.create(
            name="Test Source",
    ...
    def test_all_fields_are_optional(self):
        form = ForwardSnapshotFilterForm(data={})
        self.assertTrue(form.is_valid(), form.errors)

    def test_valid_filter_form_with_all_fields(self):
        form = ForwardSnapshotFilterForm(
            data={
                "name": "Test Snapshot",
                "status": "loaded",
...
class ForwardSourceFilterFormTestCase(TestCase):
    def test_all_fields_are_optional(self):
        form = ForwardSourceFilterForm(data={})
        self.assertTrue(form.is_valid(), form.errors)

    def test_valid_filter_form_with_status(self):
        form = ForwardSourceFilterForm(
            data={
                "status": [
...
class ForwardIngestionFilterFormTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.source = ForwardSource.objects.create(
            name="Test Source",
    ...
    def test_all_fields_are_optional(self):
        form = ForwardIngestionFilterForm(data={})
        self.assertTrue(form.is_valid(), form.errors)

    def test_valid_filter_form_with_sync(self):
        form = ForwardIngestionFilterForm(data={"sync_id": [self.sync.pk]})
        self.assertTrue(form.is_valid(), form.errors)


class ForwardIngestionMergeFormTestCase(TestCase):
    def test_remove_branch_defaults_to_true(self):
        form = ForwardIngestionMergeForm(data={"confirm": True})
        self.assertTrue(form.is_valid(), form.errors)
    ...
    def test_valid_merge_form(self):
        form = ForwardIngestionMergeForm(data={"confirm": True, "remove_branch": True})
        self.assertTrue(form.is_valid(), form.errors)


class ForwardSyncFormTestCase(TestCase):
    maxDiff = 1500
    ...
    def test_form_initialization_with_sites_no_data(self):
        ...
        form = ForwardSyncForm(initial={"sites": ["site1", "site2"]})
    ...
    def test_form_initialization_with_snapshot_no_sites_data(self):
        ...
        snapshot_no_sites = ForwardSnapshot.objects.create(
    ...
    def test_form_initialization_with_existing_instance_no_data(self):
        ...
        sync_instance = ForwardSync.objects.create(
            name="Existing Sync",
        ...
        form = ForwardSyncForm(instance=sync_instance)
    ...
    def test_htmx_boolean_field_list_values_handled(self):
        ...
        form = ForwardSyncForm(
            initial={
                ...
                "fwd_site": ["", "on"],  # fwd_ prefixed field as list
        ...
        self.assertEqual(form.initial["fwd_site"], "on")
    ...
    def test_htmx_boolean_field_single_values_unchanged(self):
        ...
        form = ForwardSyncForm(
            initial={
                ...
                "fwd_site": "on",  # Normal string value
    ...
    def test_clean_snapshot_does_not_belong_to_source(self):
        ...
        different_source = ForwardSource.objects.create(
            name="Different Source",
            type=ForwardSourceTypeChoices.LOCAL,
            url="https://different.forward.local",
            status=DataSourceStatusChoices.NEW,
        )
    ...
    def test_save_method_basic_functionality(self):
        ...
        self.assertIsInstance(sync_instance, ForwardSync)
    ...
    def test_save_method_with_fwd_parameters(self):
        """Test save method properly handles fwd_ prefixed form fields"""
        form = ForwardSyncForm(
            data={
                "name": "Test Sync FWD Params",
                "source": self.source.pk,
                "snapshot_data": self.snapshot.pk,
                "fwd_site": True,
                "fwd_interface": True,
        ...
        # Verify fwd_ parameters were stripped and stored correctly
        # All models are `False` since checkboxes must always default to False
        expected_parameters = {
            "sites": [],
            "groups": [],
            "site": True,  # Explicitly set via fwd_site
            "manufacturer": False,
            "devicetype": False,
            "devicerole": False,
            "platform": False,
            "device": False,
            "virtualchassis": False,
            "interface": True,  # Explicitly set via fwd_interface
            "macaddress": False,
            "inventoryitem": False,
            "ipaddress": False,
            "vlan": False,
            "vrf": False,
            "prefix": True,  # Explicitly set via fwd_prefix
        }
        self.assertEqual(sync_instance.parameters, expected_parameters)
    ...
    def test_save_method_with_scheduling_no_interval(self, mock_enqueue):
        ...
        saved_instance = ForwardSync.objects.get(pk=sync_instance.pk)
    ...
    def test_fieldsets_for_local_source_type(self):
        ...
        # First fieldset should be Forward Source
        self.assertEqual(fieldsets[0].name, "Forward Source")
    ...
    def test_fieldsets_for_remote_source_type(self):
        ...
        remote_source = ForwardSource.objects.create(
            name="Test Remote Source",
            type=ForwardSourceTypeChoices.REMOTE,
            url="https://remote.forward.local",
            status=DataSourceStatusChoices.NEW,
        ...
        remote_snapshot = ForwardSnapshot.objects.create(
