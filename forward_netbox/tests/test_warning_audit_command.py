...
class ForwardWarningAuditCommandTest(TestCase):
    ...
    def test_reclassifies_transient_shard_fallback_warning_as_info(self):
        ingestion = ForwardIngestion.objects.create(
            sync=self.sync,
            snapshot_selector="latestProcessed",
        ...
        ingestion.job = self._job(
            ingestion=ingestion,
            name="warning-audit-transient-shard-fallback-job",
            logs=[
                [
                    timezone.now().isoformat(),
                    "warning",
                    self.sync.name,
                    "/plugins/forward/sync/6/",
                    (
                        "Forward shard-scoped NQE fetch failed for ipam.prefix using "
                        "nqe_column_filter; falling back to full model fetch: "
    ...
    def test_reclassifies_server_error_shard_fallback_warning_as_info(self):
        ingestion = ForwardIngestion.objects.create(
            sync=self.sync,
            snapshot_selector="latestProcessed",
            snapshot_id="snapshot-6b",
        )
        ingestion.job = self._job(
            ingestion=ingestion,
            name="warning-audit-server-error-shard-fallback-job",
            logs=[
                [
                    timezone.now().isoformat(),
                    "warning",
                    self.sync.name,
                    "/plugins/forward/sync/6b/",
                    (
                        "Forward shard-scoped NQE fetch failed for ipam.prefix using "
                        "nqe_column_filter; falling back to full model fetch: "
