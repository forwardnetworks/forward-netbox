...
class ArchitectureRuntimeEvidenceTaskTest(unittest.TestCase):
    ...
    def test_run_field_scale_runtime_matrix_allows_existing_source_without_secret_env(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with (
                patch.dict(
                    os.environ,
                    {
                        "FORWARD_SMOKE_SOURCE_NAME": "smoke-source-redacted-20260601",
                        "FORWARD_SMOKE_SYNC_NAME": "smoke-sync-redacted-20260601",
                        "FORWARD_SMOKE_DATASET_LABEL": "redacted",
        ...
        self.assertEqual(payload["metadata"]["dataset_label"], "redacted")
        ...
        self.assertTrue(
            all(
                "--source-name smoke-source-redacted-20260601" in command
                for command in commands
            )
        )
    ...
    def test_run_field_scale_runtime_matrix_records_dataset_label(self):
            ...
            with (
                patch.dict(
                    os.environ,
                    {
                        ...
                        "FORWARD_SMOKE_PASSWORD": "secret",
                        "FORWARD_SMOKE_NETWORK_ID": "123",
                        "FORWARD_SMOKE_DATASET_LABEL": "REDACTED-Prod",
                        "FORWARD_FIELD_SCALE_EVIDENCE_PATH": str(artifact_path),
                    },
                    clear=True,
                ),
        ...
        self.assertEqual(status, "completed")
        self.assertEqual(evidence["status"], "passed")
        self.assertEqual(payload["metadata"]["dataset_label"], "REDACTED-Prod")
    ...
    def test_release_dataset_gate_passes_with_fresh_matching_label(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                json.dumps(
                    {
                        "generated_at": datetime.now(timezone.utc).isoformat(),
                        "status": "passed",
                        "metadata": {"dataset_label": "redacted", "resume": False},
                        "runs": [
                            {
                                "name": "run_a_branching_validate_only",
                                "ok": True,
            ...
            with patch.dict(
                ...
                tasks.release_dataset_gate.body(context, dataset_label="redacted")
                ...
                json.dumps(
                    {
                        "generated_at": datetime.now(timezone.utc).isoformat(),
                        "status": "passed",
                        "metadata": {"dataset_label": "org-staging", "resume": False},
                        "runs": [
                            {
                                "name": "run_a_branching_validate_only",
                                "ok": True,
                ...
                json.dumps(
                    {
                        "generated_at": datetime.now(timezone.utc).isoformat(),
                        "status": "passed",
                        "metadata": {"dataset_label": "redacted", "resume": True},
                        "runs": [
                            {
                                "name": "run_a_branching_validate_only",
                                "ok": True,
    ...
    def test_release_runtime_preflight_task_raises_on_failure(self):
        ...
        with (
            ...
            with self.assertRaises(Exit) as raised:
                tasks.release_runtime_preflight.body(context, dataset_label="redacted")
        self.assertEqual(raised.exception.code, 1)
    ...
    def test_release_readiness_audit_task_raises_on_failure(self):
        ...
        with patch.object(
            ...
            with self.assertRaises(Exit) as raised:
                tasks.release_readiness_audit.body(context, dataset_label="redacted")
        self.assertEqual(raised.exception.code, 1)
